...
function onUse(cid, item, fromPosition, itemEx, toPosition, isHotkey)
	...
	if item.itemid == 9992 then
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Your health has been refilled.")
	...
	elseif item.itemid == 9993 then
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You feel better body condition.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 9994 then
		doRemoveItem(item.uid, 1)
		doAddCondition(cid, condition_shield)
		doCreatureSay(cid, "Chomp.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You feel less vulnerable.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 9995 then
		doRemoveItem(item.uid, 1)
		doAddCondition(cid, condition_ml)
		doCreatureSay(cid, "Chomp.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You feel smarter.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 9996 then
		doRemoveItem(item.uid, 1)
		doCreatureSay(cid, "Slurp.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You don't really know what this did to you, but suddenly you feel very happy.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_HEARTS)
	elseif item.itemid == 9997 then
		doRemoveItem(item.uid, 1)
		doAddCondition(cid, condition_melee)
		doCreatureSay(cid, "Yum.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You feel stronger.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 9998 then
		doRemoveItem(item.uid, 1)
		doAddCondition(cid, condition_speed)
		doCreatureSay(cid, "Munch.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Your speed has been increased.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 9999 then
		doRemoveItem(item.uid, 1)
		Creature(cid):addMana(Creature(cid):getMaxMana() - Creature(cid):getMana())
		doCreatureSay(cid, "Chomp.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Your mana has been refilled.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 10000 then
		doRemoveItem(item.uid, 1)
		doAddCondition(cid, condition_dist)
		doCreatureSay(cid, "Mmmm.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You feel more focused.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 10001 then
		doRemoveItem(item.uid, 1)
		doAddCondition(cid, condition_f)
		doCreatureSay(cid, "Smack.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You felt fishing inspiration.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 12540 then
		doRemoveItem(item.uid, 1)
		setPlayerStorageValue(cid,17100,os.time() + 86400 )
		doCreatureSay(cid, "Yum.", TALKTYPE_MONSTER_SAY)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Underwater walking speed increased.")
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 12542 then
		if math.random(1,5) == 5 then
			doRemoveItem(item.uid, 1)
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You take the last gulp from the large bowl. No leftovers!")
		else
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You take a gulp from the large bowl, but there's still some blackjack in it.")
		end
		Creature(cid):addHealth(getCreatureMaxHealth(cid) - getCreatureHealth(cid))
		doCreatureSay(cid, "Gulp.", TALKTYPE_MONSTER_SAY)
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 12543 then
		...
		if r == 1 then
			...
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You feel stronger, but you have no idea what was increased.")
		elseif r == 2 then
			doSetCreatureLight(cid, 15, 154, 60*60*1000)
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You feel enlightened.")
		elseif r == 3 then
			doAddCondition(cid, condition_i)
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "You became invisible.")
		elseif r == 4 then
			Creature(cid):addHealth(getCreatureMaxHealth(cid) - getCreatureHealth(cid))
			Creature(cid):addMana(Creature(cid):getMaxMana() - Creature(cid):getMana())
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Your vitality has been restored.")
		end
		doCreatureSay(cid, "Smack.", TALKTYPE_MONSTER_SAY)
		doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_RED)
	elseif item.itemid == 12544 then
		...
		if(ring.itemid == 0) then
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "No ring equipped.")
		...
		if r_t[ring.itemid] ~= nil then
			...
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Your ring has been multiplied.")
			doCreatureSay(cid, "Slurp!", TALKTYPE_MONSTER_SAY)
		else
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "This ring cannot be multiplied.")
			doSendMagicEffect(fromPosition, CONST_ME_POFF)     
		end
	end
	return true
