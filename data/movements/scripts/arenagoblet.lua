function onStepIn(cid, item, position, fromPosition) 
	...
	if item.actionid == 42360 then 
		if getPlayerStorageValue(cid, 42360) ~= 1 then 
			...
			local goblet = doCreateItem(5807, 1, {x=gobletPos.x,y=gobletPos.y-1,z=gobletPos.z}) 
	...
	elseif item.actionid == 42370 then 
		if getPlayerStorageValue(cid, 42370) ~= 1 then 
			...
			local goblet = doCreateItem(5806, 1, {x=gobletPos.x,y=gobletPos.y-1,z=gobletPos.z}) 
	...
	elseif item.actionid == 42380 then 
		if getPlayerStorageValue(cid, 42380) ~= 1 then 
			...
			local goblet = doCreateItem(5805, 1, {x=gobletPos.x,y=gobletPos.y-1,z=gobletPos.z}) 
