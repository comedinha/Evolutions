...
bool Game::playerBrowseField(uint32_t playerId, const Position& pos, uint16_t spriteId, int16_t stackpos)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	
	return internalGetThing(player, pos, stackpos, spriteId, BROWSE_ITEM);
}