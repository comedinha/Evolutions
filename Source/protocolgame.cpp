...
void ProtocolGame::parseBrowseField(NetworkMessage& msg)
{
	...
	uint16_t spriteId = msg.get<uint16_t>();
	int16_t stackpos = msg.get<char>();
	addGameTask(&Game::playerBrowseField, player->getID(), pos, spriteId, stackpos);
}
...
void ProtocolGame::sendGoods(const ShopInfoList& shop)
{
	...
	else
	{
		...
		for(ShopInfoList::const_iterator sit = shop.begin(); sit != shop.end(); ++sit)
		{
			...
			{
				std::map<uint32_t, uint32_t>::const_iterator findIt = goodsMap.find(sit->itemId);
				if(findIt != goodsMap.end() && findIt->second > 0)
					goodsMap[sit->itemId] = findIt->second;
			}
		}
	}
...
void ProtocolGame::sendMarketBrowseOwnHistory(const HistoryMarketOfferList& buyOffers, const HistoryMarketOfferList& sellOffers)
{
	...
	msg->put<uint32_t>(std::min(800, (int32_t)buyOffers.size()));
	...
	msg->put<uint32_t>(std::min(800, (int32_t)sellOffers.size()));
