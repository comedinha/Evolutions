...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version" || (*it) == "-v")
		{
			...
			"Visit our blog for updates and resources: http://sotserv.blogspot.com.br/.\n"
			"and visit our forum for updates, support and resources: http://tibiaking.com/forum/forum/212-alissow-server/.\n"
			"This Server is FREE! // Este Servidor e de graca!\n";
			return false;
		}
...
void otserv(StringVec, ServiceManager* services)
{
	...
	std::clog << SOFTWARE_NAME << ", version " << SOFTWARE_VERSION << "" << std::endl
		...
		<< "Visit our blog for updates and resources: http://sotserv.blogspot.com.br/." << std::endl
		<< "and visit our forum for support: http://tibiaking.com/forum/forum/212-alissow-server/." << std::endl
		<< "This Server is FREE! / Este Servidor e de graca!" << std::endl << std::endl;
	std::stringstream ss;
#ifdef __DEBUG__
	ss << " GLOBAL";
#endif
	...
	mutexName << "alissowserver_" << g_config.getNumber(ConfigManager::WORLD_ID);

	CreateMutex(NULL, FALSE, mutexName.str().c_str());
	if(GetLastError() == ERROR_ALREADY_EXISTS)
		startupErrorMessage("Another instance of Alissow Server is already running with the same worldId.\nIf you want to run multiple servers, please change the worldId in configuration file.");
