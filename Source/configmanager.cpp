...
bool ConfigManager::load()
{
	...
	if(!m_loaded) //info that must be loaded one time- unless we reset the modules involved
	{
		...
		m_confString[SQL_DB] = getGlobalString("sqlDatabase", "alissowserver");
		m_confString[SQL_USER] = getGlobalString("sqlUser", "root");
		m_confString[SQL_PASS] = getGlobalString("sqlPass", "");
		m_confString[SQL_FILE] = getGlobalString("sqlFile", "alissowserver.s3db");
		...
		m_confString[MAP_NAME] = getGlobalString("mapName", "xedegux.otbm");
	...
	m_confString[MAP_AUTHOR] = getGlobalString("mapAuthor", "Alissow and Xedegux");
	...
	m_confNumber[LEVEL_TO_OFFLINE_BED] = getGlobalNumber("levelToOfflineInBed", 200);
